...
static gchar *make_url(const gchar *s) {
    ...
    if (g_str_has_prefix(t, "http://") || g_str_has_prefix(t, "https://") ||
        g_str_has_prefix(t, "file://") || g_str_has_prefix(t, "about:")) return t;
...
static void address_activate(GtkEntry *e, gpointer d) { (void)e; go_address(d); }
static void back_cb(GtkButton *b, gpointer d) { (void)b; NexoTab *t = current(d); if (t) webkit_web_view_go_back(t->view); }
static void forward_cb(GtkButton *b, gpointer d) { (void)b; NexoTab *t = current(d); if (t) webkit_web_view_go_forward(t->view); }
static void reload_cb(GtkButton *b, gpointer d) { (void)b; NexoTab *t = current(d); if (t) webkit_web_view_reload(t->view); }
static void stop_cb(GtkButton *b, gpointer d) { (void)b; NexoTab *t = current(d); if (t) webkit_web_view_stop_loading(t->view); }
static void home_cb(GtkButton *b, gpointer d) { (void)b; NexoTab *t = current(d); if (t) webkit_web_view_load_uri(t->view, HOME_URL); }
...
static void tab_click(GtkButton *b, gpointer d) {
    ...
    if (t) select_tab(d, index_of(d, t));
}
...
static gboolean policy(WebKitWebView *v, WebKitPolicyDecision *d,
    ...
    if ((modifiers & GDK_CONTROL_MASK) && navigation_type == WEBKIT_NAVIGATION_TYPE_LINK_CLICKED) {
        new_tab(a, uri);
        webkit_policy_decision_ignore(d);
        return TRUE;
    }
...
static void new_tab_cb(GtkButton *b, gpointer d) { (void)b; new_tab(d, HOME_URL); }
...
static void activate(GtkApplication *g, gpointer u) {
    ...
    gtk_box_append(GTK_BOX(toolbar), a->back);
    gtk_box_append(GTK_BOX(toolbar), a->forward);
    gtk_box_append(GTK_BOX(toolbar), a->reload);
    gtk_box_append(GTK_BOX(toolbar), a->stop);
    ...
    gtk_box_append(GTK_BOX(toolbar), a->address);
    gtk_box_append(GTK_BOX(toolbar), plus);
